    ...
    <div v-if="showUploadForm" class="modal-overlay" @click.self="showUploadForm = false">
      <div class="modal">
        <div class="modal-header">
          <h3>上传文档到集合</h3>
          <button class="btn-close" @click="showUploadForm = false">&times;</button>
        </div>
        <form @submit.prevent="triggerFileSelect">
...
export default {
  ...
  setup() {
    ...
    async function handleDelete() {
      ...
      deleting.value = true
      try {
        await deleteDocument(deleteTarget.value.file_name)
    ...
    return {
      ...
      openUploadForm, triggerFileSelect, handleFileChange,
